...
Param(
    [parameter(Mandatory = $false)] $ReleaseBaseURL="https://github.com/projectcalico/calico/releases/download/v3.19.1/",
    [parameter(Mandatory = $false)] $ReleaseFile="calico-windows-v3.19.1.zip",
...
function GetCalicoKubeConfig()
{
    ...
    echo "found server name ~ $server"
    # (Get-Content $KubeConfigPath).replace('<ca>', $ca).replace('<server>', $server.Trim()).replace('<token>', $token) | Set-Content $RootDir\calico-kube-config -Force

    Write-Output "Wrote a new calico kube config to $RootDir\calico-kube-config  if you want to use it."
}
...
function StartCalico()
{
    ...
    if (-not (Test-Path $env:CNI_CONF_DIR)) { 
        Write-Output "CNI_CONF_DIR env var not set $env:CNI_CONF_DIR, exiting"
        exit 3
    }
    if (-not (Test-Path $env:CNI_BIN_DIR)) { 
        Write-Output "CNI_BIN_DIR env var not set $env:CNI_BIN_DIR, exiting"
        exit 3
    }
    if (-not (Test-Path $env:KUBECONFIG)) { 
        Write-Output "KUBECONFIG env var not set $env:KUBECONFIG, exiting"
        exit 3
    }

    Write-Host "Ensuring CNI directories exist... bin $env:CNI_BIN_DIR and conf $env:CNI_CONF_DIR"
...
if (-Not [string]::IsNullOrEmpty($KubeVersion) -and $platform -NE "eks") {
    Write-Output "Found a kubernetes version as input, installing it..."
    PrepareKubernetes
} else {
    Write-Output "Skipping Preparation of Kubernetes, no version was requested, assuming its already there."
}
Write-Host "Download Calico for Windows release... at $ReleaseBaseURL $ReleaseFile"
try {
    DownloadFile -Url $ReleaseBaseURL/$ReleaseFile -Destination c:\calico-windows.zip
} catch [Exception] {
    Write-Output "Failed downloading calico trying again............"
    DownloadFile -Url $ReleaseBaseURL/$ReleaseFile -Destination c:\calico-windows.zip
}
...
if ($platform -EQ "aks") {
    ...
    $calicoNs = GetCalicoNamespace
    GetCalicoKubeConfig -CalicoNamespace $calicoNs -SecretName 'calico-windows' --KubeConfigPath C:\ProgramData\kubernetes\kubeconfig
}
if ($platform -EQ "eks") {
    EnableWinDsrForEKS
